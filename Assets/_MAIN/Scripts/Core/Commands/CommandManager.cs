    ...
    public class CommandManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (instance == null)
            {
                ...
                // Load all CMD_DatabaseExtension types
                Assembly assembly = Assembly.GetExecutingAssembly();
                Type[] extensionTypes = assembly.GetTypes()
                    .Where(t => t.IsSubclassOf(typeof(CMD_DatabaseExtension)))
                    .ToArray();

                foreach (Type extension in extensionTypes)
                {
                    // Check if the type has a static method named "Extend"
        ...
        private CoroutineWrapper ExecuteSubCommand(string commandName, string[] args)
        {
            ...
            // Get the last part of the command name
            ...
            if (subDatabases.ContainsKey(databaseName))
            {
                ...
                if (command != null)
                    ...
                    Debug.LogError($"Sub-command called '{subCommandName}' not found in sub database '{databaseName}'.");
            }

            string characterName = databaseName;
            //if we've reached here, then we should try to runn as a character command
            ...
            Debug.LogError($"Sub-database called '{databaseName}' not found or not exists! Command {subCommandName} could not be run.");
            return null;
        }
        ...
        private CoroutineWrapper StartProcess(string commandName, Delegate command, string[] args)
        {
            ...
            CommandProcess cmd = new CommandProcess(processID, commandName, command, null, args);
        ...
        public void KillProcess(CommandProcess cmd)
        {
            ...
            // Call the onTerminateAction if it exists
        ...
        private IEnumerator WaitingForProcessToComplete(Delegate command, string[] args)
        {
            if (command is Action)
                ...
                command.DynamicInvoke(args.Length == 0 ? string.Empty : args[0]); //simle check if args.Length = 0 then pass in an empty string other wise we pass in the data its given
                ...
                Debug.LogWarning("Unsupported command type.");
        }
        ...
        // allow databases-extensions to create sub-databases
