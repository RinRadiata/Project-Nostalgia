    ...
    public class LL_Choice : ILogicalLine
    {
        ...
        public IEnumerator Execute(DIALOGUE_LINE line)
        {
            ...
            Conversation newConversation = new Conversation
                (selectedChoice.resultLines, file: currentConversation.file,
                fileStartIndex: selectedChoice.startIndex, fileEndIndex: selectedChoice.endIndex);
            //to get the relative index of the spot in the list of the conversation where this thing actually needs to terminate at (line in the list)
        ...
        private List<Choice> GetChoicesFromData(EncapsulatedData data)
        {
            ...
            for (i = 1; i < data.lines.Count; i++)
            {
                ...
                if (IsChoiceStart(line) && encapsulationDepth == 1)
                {
                    if (!isFirstChoice)
                    {
                        choice.startIndex = data.startingIndex + (choiceIndex + 1);//to go past the choice title and get the lines
                        choice.endIndex = data.startingIndex + (i - 1);//so that to not get the choice header
                        choices.Add(choice);
                        choice = new Choice
                        {
                            title = string.Empty,
            ...
            {
                choice.startIndex = data.startingIndex + (choiceIndex + 1);//so that we can get the actual lines start
