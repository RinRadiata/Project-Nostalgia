...
namespace CHARACTERS
{
    public abstract class Character
    {
        ...
        public string characterID => config.characterID; //for profile and save data reference, should be unique for each character
        public Color color { get; protected set; } = Color.white;
        protected Color displayColor => highlighted ? hightlightedColor : unHightlightedColor; // color of character(all type) when highlighted or not highlighted
        protected Color hightlightedColor => color;
        protected Color unHightlightedColor => new Color
            (color.r * UNHIGHLIGHTED_DARKEN_STRENGTH,
             color.g * UNHIGHLIGHTED_DARKEN_STRENGTH,
             color.b * UNHIGHLIGHTED_DARKEN_STRENGTH,
             color.a);  
        public bool highlighted { get; protected set; } = true;

        protected bool facingLeft = DEFAULT_ORIENTATION_IS_FACING_LEFT; // if the character is facing left or right
        ...
        {
            Debug.Log($"Show/Hide cannot be call from a base character type");
            yield return null;
        }
        ...
        // moving character to a position
        ...
        public Coroutine Highlight(float speed = 1f, bool immediate = false)
        {
            ...
            highlighted = true;
            co_highlighting = characterManager.StartCoroutine(Highlighting(immediate, speed));
        ...
        {
            Debug.Log($"Flipping is not applicable on this character type");
            yield return null;
        }

        // Set the character to a specific priority
        public void SetPriority(int priority, bool autoSortCharactersOnUI = true)
        // if true, the character will be sorted on the UI, change to false if dont want to sort
        {
