...
public class AudioManager : MonoBehaviour
{
    ...
    public const float MUTED_VOLUME_LEVEL = -80f; // Unity's minimum volume level for audio sources

    private const string SFX_PARENT_NAME = "SFX";

    // Format string for naming SFX GameObjects, e.g., "SFX - [Explosion]"
    // Uses SFX_NAME_FORMAT_CONTAINERS to wrap the sound name in brackets.
    // If we want to change the format, we can just modify the containers and it wont be affect any of the logic
    ...
    public AudioSource PlaySoundEffect(string filePath, AudioMixerGroup mixer = null, float volume = 1, float pitch = 1, bool loop = false)
    {
        ...
        if (clip == null)
        {
            Debug.Log($"Could not load audio file '{filePath}'. Please make sure this exist!");
            return null;
        }
    ...
    public AudioSource PlaySoundEffect(AudioClip clip, AudioMixerGroup mixer = null, float volume = 1, float pitch = 1, bool loop = false, string filePath = "")
    {
        ...
        effectSource.spatialBlend = 0; // 2D sound
