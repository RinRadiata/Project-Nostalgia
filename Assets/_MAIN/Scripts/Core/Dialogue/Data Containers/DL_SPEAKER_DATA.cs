...
namespace DIALOGUE
{
    ...
    public class DL_SPEAKER_DATA
    {
        ...
        private const char EXPRESSIOLAYER_DELIMITER = ':';
        ...
        public DL_SPEAKER_DATA(string rawSpeaker)
        {
            ...
            // Populate this data to avoid null ref to values
            castName = "";
            castPosition = Vector2.zero;
            CastExpressions = new List<(int layer, string expression)>();

            // If there are no matches, then all the line here is the speaker name
            if (matches.Count == 0)
            {
                name = rawSpeaker;
                return;
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                else if (match.Value == EXPRESSIONCAST_ID)
                {
                    ...
                    // Split the castExp string by the delimiter and parse each part
                    CastExpressions = castExp.Split(EXPRESSIONLAYER_JOINER)
                        .Select(x =>
                        {
                            var parts = x.Trim().Split(EXPRESSIOLAYER_DELIMITER);
